...
A `Status` column is added. TD-1 and TD-11 become `Resolved`; TD-9 becomes `Partially resolved`
and TD-14 `Resolved` (both delivered by `teamtask-testing-emulator-strategy`); every other item
stays `Open`.
