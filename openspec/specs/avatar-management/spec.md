...
This capability defines the canonical avatar domain for the existing XML,
...
Before this capability was specified there was no single avatar authority; this capability
replaced that split with the single authority defined below. The pre-change observables
were: `AvatarViewModel` constructed `AvatarRepositorioLocal` directly, bypassing
`LocalizadorServicios` (`viewmodel/AvatarViewModel.kt:14-16`); the local repository stored
files under `filesDir/avatars/` and recorded a path in the SharedPreferences namespace
`tfg_prefs` (`data/local/AvatarRepositorioLocal.kt:11,23-34,86`); `AvatarRepositorioFirebase`
was dead code with zero callers in `app/src/main/java`
...
member cards always fell back to the placeholder; and `usuarios.avatarUrl` existed in the
model and contract (`modelo/Usuario.kt:18`;
`openspec/specs/firestore-contracts/spec.md`) but no mapper read it, so the value was
silently dropped at all four `AuthRepositorioFirebase` mapping sites
(`data/firebase/AuthRepositorioFirebase.kt:125-136,194-205,375-383,404-412`).
