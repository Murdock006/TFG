...
import es.sintaxys.teamtask.repositorio.RepositorioPareja
...
class ParejaViewModel(application: Application, private val repo: RepositorioPareja = RepositorioPareja()) : AndroidViewModel(application) {
    ...
    fun buscarInvitacionesPorCorreo(correo: String, onResult: (Result<List<es.sintaxys.teamtask.modelo.Invitacion>>) -> Unit) {
        viewModelScope.launch {
            try {
