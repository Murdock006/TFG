...
import es.sintaxys.teamtask.modelo.Categoria
...
class CategoriasRepositorio(private val context: Context) {
    ...
    suspend fun cargarCategoriasDesdeRaw(): List<Categoria> = withContext(Dispatchers.IO) {
        val raw = context.resources.openRawResource(es.sintaxys.teamtask.R.raw.categorias_sugeridas).bufferedReader().use { it.readText() }
